                ...
                <div class="row">
                    ...
                    <div class="col-12">
                        <button type="submit" class="btn btn-success" disabled={isSubmitting}>
                            ...
                            {:else}
                                <FontAwesomeIcon icon={faSave} fixedWidth /> Atualizar Projeto
                            {/if}
                        </button>
                    </div>
                </div>
...
<script lang="ts">
    ...
    let project: IProject = { label: "", user_id: null };
    ...
    const submitForm = (e: SubmitEvent) => {
        ...
        PrivateApi.put(`/projects/${project_id}`, project).then((res: AxiosResponse<IProject>) => {
            Alert({
                icon: "success",
                title: "Sucesso",
                text: `O projeto ${project?.label} foi aualizado com sucesso.`
            }).then(() => {
                goto("/admin/projects");
            });
        }).catch((err: AxiosError<IApiError>) => {
            Alert({
                icon: "error",
                title: "Erro",
                text: err.response?.data.message ?? "Ocorreu um erro durante a atualização do projeto. Por favor, tente novamente."
            });
        }).finally(() => {
            isSubmitting = false;
        });
